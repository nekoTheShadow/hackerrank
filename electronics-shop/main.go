...
func getMoneySpent(keyboards []int32, drives []int32, b int32) int32 {
	...
	for _, keyboard := range keyboards {
		for _, drive := range drives {
			yen := keyboard + drive
			if yen <= b {
				yens = append(yens, yen)
			}
		}
	}

	sort.Slice(yens, func(i, j int) bool {
		return yens[i] > yens[j]
	})
	if len(yens) == 0 {
		return -1
	} else {
...
func exec(stdin *Stdin, stdout *Stdout) {
	...
	n := stdin.ReadInt()
	m := stdin.ReadInt()
	keyboards := make([]int32, n)
	drivers := make([]int32, m)
	for i := 0; i < n; i++ {
		keyboards[i] = int32(stdin.ReadInt())
	}
	for i := 0; i < m; i++ {
		drivers[i] = int32(stdin.ReadInt())
	}
	stdout.Println(getMoneySpent(keyboards, drivers, int32(b)))
}
